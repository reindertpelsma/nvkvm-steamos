...
| Games | Shadow of the Tomb Raider and Portal 2 launch and play under the broker (screenshots above). Planetary Annihilation Titans launches and runs, but Chromium's GPU watchdog terminates it after ~20 minutes when forwarding is slow ([status.md](docs/status.md), `GpuWatchdogThread::DeliberatelyTerminateToRecoverFromHang`) -- it is not yet a title you can play through. Minecraft is playable under SDL on a non-SteamOS Linux guest. **Windows titles through Proton do not work if they need D3D12:** seven Vulkan device extensions, including the ray-tracing ones vkd3d-proton enables to expose DXR, make `vkCreateDevice` fail in the guest (MEASURED 2026-09-01 with Red Dead Redemption 2, which dies at ERR_GFX_INIT; the same probe succeeds on the host). Every title above is native Linux/Vulkan, which is why this went unnoticed. |
