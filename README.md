...
| Games | Shadow of the Tomb Raider and Portal 2 launch and play under the broker (screenshots above). Planetary Annihilation Titans launches and runs, but Chromium's GPU watchdog terminates it after ~20 minutes when forwarding is slow ([status.md](docs/status.md), `GpuWatchdogThread::DeliberatelyTerminateToRecoverFromHang`) -- it is not yet a title you can play through. Minecraft is playable under SDL on a non-SteamOS Linux guest. Windows titles through Proton are **not yet a claim we make** -- Red Dead Redemption 2 was tried on 2026-09-01 and its D3D12 launcher initially failed, which turned out to be a guest-staging bug (libcuda was never installed on an Arch-family guest), now fixed in nvkvm-pv. Whether the game plays through has not been measured, so it is not claimed here. |
