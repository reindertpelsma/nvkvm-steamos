...
| Games | Shadow of the Tomb Raider and Portal 2 launch and play under the broker (screenshots above). Planetary Annihilation Titans launches and runs, but Chromium's GPU watchdog terminates it after ~20 minutes when forwarding is slow ([status.md](docs/status.md), `GpuWatchdogThread::DeliberatelyTerminateToRecoverFromHang`) -- it is not yet a title you can play through. Minecraft is playable under SDL on a non-SteamOS Linux guest. **Red Dead Redemption 2 plays** — a Windows D3D12 title through Proton and vkd3d-proton, which is a different path from every native-Vulkan title above and had never worked here until 2026-09-01. Measured on an RTX 4070 / driver 595.84, fullscreen with the broker grab active: GPU 28–39%, VRAM ~5.07 GB, 49–65 W, sustained 2.2–2.7 GHz boost, 41 °C. The host reports 38–39% and ~5.08 GB for the same GPU at the same moment — the guest is reading the real device, not a synthesised one. Not a frame-rate claim: nothing here measures frame timing, and the low utilisation at a smooth-looking frame rate suggests a vsync cap rather than a GPU-bound scene. |
